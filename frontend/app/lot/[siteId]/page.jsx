...
import { SITES_REGISTRY, ACTIVE_TICKETS } from "../../../lib/mock-data";
import { notFound } from "next/navigation";
import Image from "next/image";
import Link from "next/link";

export default function LotMobileClientPage({ params }) {
  const resolvedParams = use(params);
  const siteId = resolvedParams.siteId;
  const site = SITES_REGISTRY[siteId];
  ...
  const handleCalculateExit = (e) => {
    ...
    const slabRate = site.rates?.[ticket.vehicleType]?.price || (ticket.vehicleType === "4W" ? 20 : 10);
    const computedTotal = Math.max(Math.ceil(totalHours / 2) * slabRate, slabRate);
              ...
              <form onSubmit={handleCheckIn} className="space-y-4">
                ...
                <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-3.5 space-y-2 text-xs">
                  <div className="flex justify-between items-center pb-1.5 border-b border-slate-200">
                    <span className="text-slate-400 text-[11px]">Leased Operator</span>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-600">
                    ...
                    <div>
                      ...
                      <span className="font-medium text-slate-700">{site.tenure || "3 Years"}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[10px]">Parking Area</span>
                      <span className="font-medium text-slate-700">{site.areaSqft || "14,200"} sqft</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[10px]">Slot Validity</span>
                      <span className="font-medium text-slate-700">{site.slotValidity || "Active"}</span>
                    </div>
                  </div>
                ...
                {/* Adjusted View Work Order Button */}
                <Link
                  href="/image/Adobe Scan 16 Sept 2026.pdf" target="_blank" rel="noopener noreferrer"
